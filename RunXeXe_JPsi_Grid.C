...
AliAnalysisTaskXeXeJPsiTree_Dimuon* AddTaskXeXeJPsiTree_Dimuon_Grid(Int_t );
AliAnalysisTaskXeXeJPsi_Dimuon* AddTaskXeXeJPsi_Dimuon_Grid(Int_t );
#endif

void RunXeXe_JPsi_Grid(
                     const char *RunMode = "full",
                     //Int_t RunNumber = 280234,
                     Int_t RunNumber = 246980,
		     Bool_t usePhysicsSelection=kTRUE,
                     //TString GridDir="/alice/data/2017/LHC17n",
                     //TString DataPattern="*muon_calo_pass2/AOD/*/AliAOD.root",
                     TString GridDir="/alice/data/2015/LHC15o",
                     TString DataPattern="*muon_calo_pass1/AOD197/*/AliAOD.root",
	             TString AliPhysicsVersion="vAN-20171216-1",
  ...
  //------------------------------------------------------------------------------------
  gROOT->LoadMacro("AliAnalysisTaskXeXeJPsiTree_Dimuon.cxx++g");  //NEEDED to run LOCALLY!!!!
  gROOT->LoadMacro("AddTaskXeXeJPsiTree_Dimuon_Grid.C");
  AliAnalysisTaskXeXeJPsiTree_Dimuon* taskTree = AddTaskXeXeJPsiTree_Dimuon_Grid(RunNumber);
  mgr->AddTask(taskTree);

//   //------------------------------------------------------------------------------------
//   // Add task to fill Dimuon Histos -> voglio creare solo dei tree
//   //------------------------------------------------------------------------------------
   //gROOT->LoadMacro("AliAnalysisTaskXeXeJPsi_Dimuon.cxx++g");  // DON'T use double '+' when running multiple times: it uselessly recompiles everything!
   //gROOT->LoadMacro("AddTaskXeXeJPsi_Dimuon_Grid.C");
   //AliAnalysisTaskXeXeJPsi_Dimuon *taskHisto = AddTaskXeXeJPsi_Dimuon_Grid(RunNumber);
   //mgr->AddTask(taskHisto);
